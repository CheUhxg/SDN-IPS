/*****************************************************
 *  功能：p4自定义处理逻辑，DDoS检测与攻击缓解+服务链控制转发 *
 *  赛项:网络技术挑战赛A-ST                             *
 *  语言:p4_16                                       *
 ****************************************************/
#include <core.p4>
...
control ingress(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    /*
     * 计算草图声明。
     * “Src”和“Dst”表示草图近似计算源 IP 地址或目标 IP 地址。
     * “Curr”、“Last”和“Safe”表示草图所指的观察窗口的类型。
     * 分别表示：当前窗口、最后窗口、安全窗口
     * 由于 P4 不提供矩阵或记录，每个草图行需要两个草图，
     * 一个用于计数器，一个用于观察窗口 ID 注释用于识别是否是同一个观察窗口。
     */
    ...
    // Entropy EWMA指数加权移动平均线 and EWMMD指数加权移动平均差 - Fixed point representation: 14 integer bits, 18 fractional bits.
        ...
        if (hdr.ipv4.isValid()) {
           ...
           // Row 1 Estimate
            ...
            if (classification == LEGITIMATE) {
                clone3(CloneType.I2E, SFC_SESSION, standard_metadata);
